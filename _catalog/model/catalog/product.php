...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, 
			...
			(SELECT image FROM " . DB_PREFIX . "product_image pi WHERE pi.product_id = p.product_id ORDER BY pi.sort_order ASC, pi.product_image_id ASC LIMIT 1) AS image2, 
			p.sort_order 
      ...
      if (!empty($data['filter_manufacturer_id'])) {
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'pd.name' || $data['sort'] == 'p.model') {
				$sql .= " ORDER BY IF(p.price > 0, 0, 1), LCASE(" . $data['sort'] . ")";
			} elseif ($data['sort'] == 'p.price') {
				$sql .= " ORDER BY IF(p.price > 0, 0, 1), (CASE WHEN special IS NOT NULL THEN special WHEN discount IS NOT NULL THEN discount ELSE p.price END)";
			} else {
				$sql .= " ORDER BY IF(p.price > 0, 0, 1), " . $data['sort'];
			}
		} else {
			$sql .= " ORDER BY IF(p.price > 0, 0, 1), p.sort_order";
		}
	...
	public function getProductPair($product_info) {
		$pairs = [];
		
		$q = $this->db->query("SELECT product_id, model FROM " . DB_PREFIX . "product WHERE sku = '" . $this->db->escape($product_info['sku']) . "' ORDER BY model");
		foreach ($q->rows as $row) {
			$model = explode('_', $row['model']);
