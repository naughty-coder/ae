...
class ControllerStartupSeoPro extends Controller {
	...
	private function getPathByCategory($category_id) {
	    ...
	    if ($category_id < 1) return null; // Замість false повертаємо null

	    static $path = null;
	    if (!isset($path)) {
	        $path = $this->cache->get('category.seopath');
	        if (!is_array($path)) $path = []; // Забезпечуємо, що $path завжди є масивом
	    }
	    ...
	    if (!isset($path[$category_id])) {
	        ...
	        $path[$category_id] = $query->num_rows ? $query->row['path'] : null; // Замість false повертаємо null
